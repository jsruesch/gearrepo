<?php
/*
 Template Name: Projects
*/
?>

<?php get_header(); ?>
<?php include ('nav.php'); ?>

<!--
  BEGIN: Main Content
-->

  <div class="container-fluid mar10-top mar20-bot pad30-left pad30-right">
    <div class="container">
      <div class="row">
        <!-- At Play in the Cosmos -->
        <div class="col-sm-6 pad10">
          <a href="/at-play-in-the-cosmos">
            <div class="row selproj-btn">
              <div class="row project-page-image">
                <img src="<?php echo get_template_directory_uri(); ?>/img/projects/cosmos/project-page-image-cosmos-01.png"/>
              </div>
              <div class="row bg-ddblue pad25">
                <h3 class="text-green">At Play in the Cosmos</h3>
                <p class="text-lblue">
                    Astronomy
                </p>
              </div>
            </div>
            </a>
        </div><!-- END: At Play in the Cosmos -->
        <!-- Econauts -->
        <div class="col-sm-6 pad10">
          <a href="/microsites/econauts/" target="_blank">
            <div class="row selproj-btn">
              <div class="row project-page-image">
                <img src="<?php echo get_template_directory_uri(); ?>/img/projects/econauts/project-page-image-econauts-01.png"/>
              </div>
              <div class="row bg-ddblue pad25">
                <h3 class="text-green">Econauts</h3>
                <p class="text-lblue">
                    Environmental Science
                </p>
              </div>
            </div>
            </a>
        </div><!-- Econauts -->
        <!-- Oztoc Expedition -->
        <div class="col-sm-6 pad10">
          <a href="<?php echo get_template_directory_uri(); ?>/microsites/oztoc/" target="_blank">
            <div class="row selproj-btn">
              <div class="row project-page-image">
                <img src="<?php echo get_template_directory_uri(); ?>/img/projects/oztoc/project-page-image-oztoc-01.png"/>
              </div>
              <div class="row bg-ddblue pad25">
                <h3 class="text-green">Oztoc Expedition</h3>
                <p class="text-lblue">
                    Circuitry and Programming Design
                </p>
              </div>
            </div>
            </a>
        </div><!-- END: Oztoc Expedition -->
        <!-- Progenitor X -->
        <div class="col-sm-6 pad10">
          <a href="<?php echo get_template_directory_uri(); ?>/microsites/progenitor/" target="_blank">
            <div class="row selproj-btn">
              <div class="row project-page-image">
                <img src="<?php echo get_template_directory_uri(); ?>/img/projects/progenitor/project-page-image-progenitor-01.png"/>
              </div>
              <div class="row bg-ddblue pad25">
                <h3 class="text-green">Progenitor X</h3>
                <p class="text-lblue">
                    Stem Cell Science
                </p>
              </div>
            </div>
            </a>
        </div><!-- END: Progenitor X -->
        <!-- Crystals of Kaydor -->
        <div class="col-sm-6 pad10">
          <a href="<?php echo get_template_directory_uri(); ?>/microsites/kaydor/" target="_blank">
            <div class="row selproj-btn">
              <div class="row project-page-image">
                <img src="<?php echo get_template_directory_uri(); ?>/img/projects/crystals/project-page-image-crystals-01.png"/>
              </div>
              <div class="row bg-ddblue pad25">
                <h3 class="text-green">Crystals of Kaydor</h3>
                <p class="text-lblue">
                    Pro-Social Mindfulness
                </p>
              </div>
            </div>
            </a>
        </div><!-- END: Crystals of Kaydor -->
        <!-- DeVry Medical -->
        <div class="col-sm-6 pad10">
          <a href="">
            <div class="row selproj-btn">
                <div class="row project-page-image">
                  <img src="<?php echo get_template_directory_uri(); ?>/img/projects/rosstide/project-page-image-rosstide-01.png"/>
                </div>
              <div class="row bg-ddblue pad25">
                <h3 class="text-green">Rosstide</h3>
                <p class="text-lblue">
                    Medical Anatomy
                </p>
              </div>
            </div>
            </a>
        </div><!-- END: DeVry Medical -->
        <!-- DeVry Veterinary -->
        <div class="col-sm-6 pad10">
          <a href="">
            <div class="row selproj-btn">
                <div class="row project-page-image">
                  <img src="<?php echo get_template_directory_uri(); ?>/img/projects/pawpad/project-page-image-pawpad-01.png"/>
                </div>
              <div class="row bg-ddblue pad25">
                <h3 class="text-green">Pawpad</h3>
                <p class="text-lblue">
                    Veterinary Anatomy
                </p>
              </div>
            </div>
            </a>
        </div><!-- END: DeVry Veterinary -->
        <!-- Vetruvian Table -->
        <div class="col-sm-6 pad10">
          <a href="">
            <div class="row selproj-btn">
                <div class="row project-page-image">
                  <img src="<?php echo get_template_directory_uri(); ?>/img/projects/apex/project-page-image-apex-01.png"/>
                </div>
              <div class="row bg-ddblue pad25">
                <h3 class="text-green">Apex Anatomy Table</h3>
                <p class="text-lblue">
                    Interactable Anatomy Table
                </p>
              </div>
            </div>
            </a>
        </div><!-- END: DeVry Medical -->
        <!-- Tenacity -->
        <div class="col-sm-6 pad10">
          <a href="">
            <div class="row selproj-btn">
              <div class="row project-page-image">
                <img src="<?php echo get_template_directory_uri(); ?>/img/projects/tenacity/project-page-image-tenacity-01.png"/>
              </div>
              <div class="row bg-ddblue pad25">
                <h3 class="text-green">Tenacity</h3>
                <p class="text-lblue">
                    Mindfulness and Meditation
                </p>
              </div>
            </div>
            </a>
        </div><!-- END: Tenacity -->
        <!-- Fair Play -->
        <div class="col-sm-6 pad10">
          <a href="http://fairplaygame.org" target="_blank">
            <div class="row selproj-btn">
              <div class="row project-page-image">
                <img src="<?php echo get_template_directory_uri(); ?>/img/projects/fairplay/project-page-image-fairplay-01.png"/>
              </div>
              <div class="row bg-ddblue pad25">
                <h3 class="text-green">Fair Play</h3>
                <p class="text-lblue">
                    Complex Social Bias
                </p>
              </div>
            </div>
            </a>
        </div><!-- END: Fair Play -->
        <!-- Soul of a Place -->
        <div class="col-sm-6 pad10">
         <a href="<?php echo get_template_directory_uri(); ?>/microsites/soul/" target="_blank">
            <div class="row selproj-btn">
              <div class="row project-page-image">
                <img src="<?php echo get_template_directory_uri(); ?>/img/projects/soul/project-page-image-soul-01.png"/>
              </div>
              <div class="row bg-ddblue pad25">
                <h3 class="text-green">Soul of a Place</h3>
                <p class="text-lblue">
                    Federal Writers Project
                </p>
              </div>
            </div>
            </a>
        </div><!-- END: Soul of a Place -->
        <!-- At Play in the Cosmos -->
        <div class="col-sm-6 pad10">
          <a href="<?php echo get_template_directory_uri(); ?>/microsites/virulent/" target="_blank">
            <div class="row selproj-btn">
              <div class="row project-page-image">
                <img src="<?php echo get_template_directory_uri(); ?>/img/projects/virulent/project-page-image-virulent-01.png"/>
              </div>
              <div class="row bg-ddblue pad25">
                <h3 class="text-green">Virulent</h3>
                <p class="text-lblue">
                    Virology
                </p>
              </div>
            </div>
            </a>
        </div><!-- END: At Play in the Cosmos -->
        <!-- At Play in the Cosmos -->
        <div class="col-sm-6 pad10">
          <a href="">
            <div class="row selproj-btn">
              <div class="row project-page-image">
                <img src="<?php echo get_template_directory_uri(); ?>/img/projects/citizen-science/project-page-image-citizen-science-01.png"/>
              </div>
              <div class="row bg-ddblue pad25">
                <h3 class="text-green">Citizen Science</h3>
                <p class="text-lblue">
                    Ecology
                </p>
              </div>
            </div>
            </a>
        </div><!-- END: At Play in the Cosmos -->
        <!-- At Play in the Cosmos -->
        <div class="col-sm-6 pad10">
          <a href="https://external-wiki.terc.edu/display/ZOOM/The+Game" target="_blank">
            <div class="row selproj-btn">
                <div class="row project-page-image">
                  <img src="<?php echo get_template_directory_uri(); ?>/img/projects/zoombinis/project-page-image-zoombinis-01.png"/>
                </div>
              <div class="row bg-ddblue pad25">
                <h3 class="text-green">Zoombinis</h3>
                <p class="text-lblue">
                    Logic & Puzzle Solving
                </p>
              </div>
            </div>
            </a>
        </div><!-- END: At Play in the Cosmos -->
      </div>
    <!-- END: UNIFIED VISION -->

    </div><!-- /.container -->
  </div><!-- /.container-fluid -->
<!-- END: Main Content -->

<?php get_footer(); ?>
